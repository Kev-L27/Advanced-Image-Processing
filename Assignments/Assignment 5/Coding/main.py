...
# image defines the camera calibration. 
#
...
def main(args):
    ...
    if args.image == "mt_rushmore":
        ...
        #pic_a = np.array(Image.fromarray(pic_a).resize((int(sf*pic_a.shape[0]), int(sf*pic_a.shape[1])), resample=PIL.Image.BILINEAR))
        im = Image.fromarray(pic_a)
        size = (np.array(im.size)*sf).astype(int)
