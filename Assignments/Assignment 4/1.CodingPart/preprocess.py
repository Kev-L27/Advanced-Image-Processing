...
class Datasets():
    ...
    def calc_mean_and_std(self):
        ...
        mean1 = 0
        std1 = [None]*data_sample.shape[0]
        mean2 = 0
        std2 = [None]*data_sample.shape[0]
        mean3 = 0
        std3 = [None]*data_sample.shape[0]

        for count,i in enumerate(data_sample):
            mean1 += np.mean(i[:,:,0])
            std1[count] = i[:,:,0]
            mean2 += np.mean(i[:,:,1])
            std2[count] = i[:,:,1]
        ...
        self.mean[0] = mean1/data_sample.shape[0]
        self.mean[1] = mean2/data_sample.shape[0]
        self.mean[2] = mean3/data_sample.shape[0]
        self.std[0] = np.std(std1)
        self.std[1] = np.std(std2)
        self.std[2] = np.std(std3)
