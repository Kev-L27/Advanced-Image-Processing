...
def get_tiny_images(image_paths):
    ...
    N = len(image_paths)
    tiny_square = 16 #Resize the original image to tiny_square*tiny_square (16*16 in this case) square resolution
    result = np.zeros((N,tiny_square**2)) # N x 256 array in this case
    ...
    for i, path in enumerate(image_paths):
        ...
        if len(img.shape) == 3:
            img = rgb2gray(img)
        img_resize = resize(img, (tiny_square, tiny_square), anti_aliasing=True)
        img_resize = np.asarray((img_resize - np.mean(img_resize))/np.std(img_resize))
...
def build_vocabulary(image_paths, vocab_size):
    ...
    '''
    '''
    z = 3 and x = 6 with size (120,60) and neighbor of k = 1 gives accuracy 40.333%
    z = 3 and x = 6 with size (120,60) and neighbor of k = 3 gives accuracy 37.867%
    z = 3 and x = 6 with size (120,60) and neighbor of k = 5 gives accuracy 41.533%
    z = 3 and x = 6 with size (120,60) and neighbor of k = 7 gives accuracy 42.933%
    '''

    #TODO: Implement this function!
    z = 3 #Each block is made up of z*z cells
    x = 4 #Each cell contains x*x pixels

    bag_of_word = []
    for i in image_paths:
        img = imread(i)
        img = resize(img, (120,60)) 
        hist = hog(img, cells_per_block=(z,z), pixels_per_cell=(x,x), feature_vector=True)
        hist = hist.reshape(-1, z*z*9)
        #bag_of_word = np.concatenate((bag_of_word, hist))
        bag_of_word.append(hist)

    #temp = np.asarray(bag_of_word)
    #print("---: ", temp.shape)
    bag_of_word = np.vstack(bag_of_word)
    #print("---: ", bag_of_word.shape)
    k_means = KMeans(vocab_size, max_iter=100)
    #k_means = KMeans(150, max_iter=100)
    k_means = k_means.fit(bag_of_word)
...
def get_bags_of_words(image_paths):
    ...
    num_path = len(image_paths)
    print('Loaded vocab from file.')
    z = 3 #Each block is made up of z*z cells
    x = 4 #Each cell contains x*x pixels

    result = np.zeros((num_path, num_vocab))
    for i, path in enumerate(image_paths):
        img = imread(path)
        img = resize(img, (120,60))
        hist = hog(img, cells_per_block=(z,z), pixels_per_cell=(x,x), feature_vector=True)
        ...
        vocab_dist = cdist(hist, vocab)
        sorted_dist = np.argsort(vocab_dist)
        for j in sorted_dist:
            min_dist = j[0] #First index of every distance vector, which is the index of the smallest
            result[i, min_dist] += 1
...
def svm_classify(train_image_feats, train_labels, test_image_feats):
    ...
    # TODO: Implement this function!
    one_vs_all = LinearSVC(tol=1e-5, C=0.005, max_iter=2000, random_state=0)
    one_vs_all.fit(train_image_feats, train_labels)
    result = one_vs_all.predict(test_image_feats)
    return result
...
def nearest_neighbor_classify(train_image_feats, train_labels, test_image_feats):
    ...
    '''
    k = 1 --> Accuracy = 23.2% for tiny_image
    k = 3 --> Accuracy = 21.333% for tiny_image
    k = 5 --> Accuracy = 21.4% for tiny_image
    k = 7 --> Accuracy = 21.2 for tiny_image
    '''
    k = 7
    ...
    for euc_dist in distances:
        label = []
        sorted_dist = np.argsort(euc_dist)
        for i in range(k):
            label.append(train_labels[sorted_dist[i]])
        nearest_neighbor = mode(label)
        result.append(nearest_neighbor[0][0])
