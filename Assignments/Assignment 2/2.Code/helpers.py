...
# Ground Truth solution for the interest points you
...
def evaluate_correspondence(img_A, img_B, ground_truth_correspondence_file,
	...
	total_student_correspondences_kept = 50, filename="eval_corr.jpg"):
	...
	matches = matches[:total_student_correspondences_kept:,:]
	confidences = confidences [:total_student_correspondences_kept]
	x1_matches = x1_matches[:total_student_correspondences_kept:]
	y1_matches = y1_matches[:total_student_correspondences_kept:]
	x2_matches = x2_matches[:total_student_correspondences_kept:]
	y2_matches = y2_matches[:total_student_correspondences_kept:]
	...
	# Decrease these values for a stricter (and more realistic evaluation)
	uniqueness_dist = 150
	good_match_dist = 150
	...
	# Used to keep track of which points the student has matched
	# to so the student only gets credit for matching a point once
	correct_matches = np.zeros(x2.shape[0])

	inds = np.asarray(range(total_student_correspondences_kept))
	good_student_match = np.zeros(total_student_correspondences_kept)
	top100_student_match = np.zeros(total_student_correspondences_kept)
	...
	for i in range(x1.shape[0]):
		...
		# 5. Find metrics that actually make sense given student matches
		kept_inds = kept_inds[good]
		if kept_inds.shape[0]>0:
			for student_match_ind in kept_inds:
				good_student_match[student_match_ind] = 1
				if i<100:
		 			top100_student_match[student_match_ind] = 1

		# 6. Find metrics that are mainly about how well you picked out the ground truth
		if np.sum(good) >= 1.0:
			correct_matches[i] = 1
	...
	precision = sum(good_student_match)/ total_student_correspondences_kept * 100.0
	top_100_precision = sum(top100_student_match)/total_student_correspondences_kept * 100.0

	print(str(int(np.sum(good_student_match))) + " of your method's top " + str(total_student_correspondences_kept) + " correspondences were in the neighborhood of the ground truth matches, and you completely missed " + str(bad_match_counter) + " of the ground truth matches.")
	print("Your method's top " + str(total_student_correspondences_kept) + " correspondences had a " + str(precision) + "% precision on the whole ground truth")
	print("Your method's top " + str(total_student_correspondences_kept) + " correspondences had a " + str(top_100_precision) + "% precision on the top 100 ground truth matches")
